...
func parseAatLookupTable0(src []byte, valuesCount int) (aatLookupTable0, int, error) {
	...
	n := 0
	{
		...
		if L := len(src); L < valuesCount*2 {
			return aatLookupTable0{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", valuesCount*2, L)
		}
...
func parseAatLookupTable10(src []byte) (aatLookupTable10, int, error) {
	...
	{
		if L := len(src); L < 6 {
			return aatLookupTable10{}, 0, fmt.Errorf("EOF: expected length: 2, got %d", L)
		}
		arrayLength := int(binary.BigEndian.Uint16(src[4:]))
		n += 2

		if L := len(src); L < 6+arrayLength*2 {
			return aatLookupTable10{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", 6+arrayLength*2, L)
		}
...
func parseAatLookupTable2(src []byte) (aatLookupTable2, int, error) {
	...
	}
	{
		...
		if L := len(src); L < 10+arrayLength*6 {
			return aatLookupTable2{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", 10+arrayLength*6, L)
		}
...
func parseAatLookupTable4(src []byte) (aatLookupTable4, int, error) {
	...
	}
	{
		...
		if L := len(src); L < 10+arrayLength*6 {
			return aatLookupTable4{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", 10+arrayLength*6, L)
		}
...
func parseAatLookupTable6(src []byte) (aatLookupTable6, int, error) {
	...
	}
	{
		...
		if L := len(src); L < 10+arrayLength*4 {
			return aatLookupTable6{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", 10+arrayLength*4, L)
		}
...
func parseMorxChain(src []byte) (morxChain, int, error) {
	...
	}
	{
		...
		if L := len(src); L < 16+arrayLength*12 {
			return morxChain{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", 16+arrayLength*12, L)
		}
...
func parseMorxSubtableNonContextual(src []byte, valuesCount int) (morxSubtableNonContextual, int, error) {
	...
	n := 0
	{
		...
		item.table, read, err = parseAatLookup(src, valuesCount)
		if err != nil {
