...
func ParseHmtx(src []byte, hmetricsLength int, leftsidebearingsLength int) (Hmtx, int, error) {
	var item Hmtx
	n := 0
	{
		subSlice := src[n:]
		if L := len(subSlice); L < +hmetricsLength*4 {
			return Hmtx{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", +hmetricsLength*4, L)
		}
	...
	}
	{
		...
		if L := len(subSlice); L < +leftsidebearingsLength*2 {
			return Hmtx{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", +leftsidebearingsLength*2, L)
		}
