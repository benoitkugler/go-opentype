...
func ParseName(src []byte) (Name, int, error) {
	...
	{
		if L := len(src); L < 4 {
			return Name{}, 0, fmt.Errorf("EOF: expected length: 4, got %d", L)
		}
	...
	{
		if L := len(src); L < 6 {
			return Name{}, 0, fmt.Errorf("EOF: expected length: 2, got %d", L)
		}
		offset := int(binary.BigEndian.Uint16(src[4:]))
		n += 2
		if L := len(src); L < offset {
			return Name{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", offset, L)
		}
	...
	}
	{
		...
		if L := len(src); L < 6+arrayLength*12 {
			return Name{}, 0, fmt.Errorf("EOF: expected length: %d, got %d", 6+arrayLength*12, L)
		}
