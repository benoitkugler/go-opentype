...
func ParseAATLoopkupExt8(src []byte) (AATLoopkupExt8, int, error) {
	...
	n := 0
	if L := len(src); L < 6 {
		return item, 0, fmt.Errorf("reading AATLoopkupExt8: "+"EOF: expected length: 6, got %d", L)
	}
	_ = src[5] // early bound checking
	item.version = binary.BigEndian.Uint16(src[0:])
...

	{
		...
		if L := len(src); L < 6+arrayLengthValues*2 {
			return item, 0, fmt.Errorf("reading AATLoopkupExt8: "+"EOF: expected length: %d, got %d", 6+arrayLengthValues*2, L)
		}
